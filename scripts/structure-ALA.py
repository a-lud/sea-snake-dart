...
struct = ipa.structure(name="ALA", data=data, imap=imap, minmap=minmap, mincov=0.9)
...
etable.to_csv(f"ALA-burnin_{burnin}-numreps_{numreps}.csv")
...
for i in range(2, 6):
    ...
    table.to_csv(f"ALA-structure-k{i}-burnin_{burnin}-numreps_{numreps}.plot.csv")
