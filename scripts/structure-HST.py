...
struct = ipa.structure(name="HST", data=data, imap=imap, minmap=minmap, mincov=0.9)
...
etable.to_csv(f"HST-burnin_{burnin}-numreps_{numreps}.csv")
...
for i in range(2, 6):
    ...
    table.to_csv(f"HST-structure-k{i}-burnin_{burnin}-numreps_{numreps}.plot.csv")
