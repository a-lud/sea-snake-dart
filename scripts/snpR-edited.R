custom_calc_genetic_distances <- function(x, facets = NULL, method = "Edwards", interpolate = "bernoulli") {
    # ============sanity checks=========
    msg <- character()
    ...
    if (sample_facets_detected) {
        ...
        out <- lapply(x, function(y) {
            lapply(y, function(z) {
                if (is.matrix(z)) {
                    custom_get_dist(z, method = method)
                } else {
...
custom_get_dist <- function(x, method = "Edwards") {
    if (method == "Edwards") {
...
custom_ibd <- function (x, facets = NULL, x_y = c("x", "y"), genetic_distance_method = "Edwards",
                    interpolate = "bernoulli", ...)
{
    ...
    # if (length(bad.xy) > 0) {
    #     msg <- c(msg, paste0("Some coordinates (arument x_y) not found in sample metadata: ",
    #                          paste(x_y[bad.xy])))
    # }
    ...
    cs <- .check_calced_stats(
        x, facets,
        paste0("genetic_distance_",genetic_distance_method, "_", interpolate)
    )
    missing <- which(!unlist(cs))
    if (length(missing) > 0) {
        x <- calc_genetic_distances(x, names(cs)[missing], genetic_distance_method,
                                    interpolate)
    }
    ...
    for (i in 1:length(geo_mats)) {
        categories <- .get.task.list(x, names(geo_mats)[i])[,
                                                            1:2, drop = F]
        if (all(categories[, 1] == ".base")) {
            geo_mats$.base[[genetic_distance_method]] <- stats::dist(x@sample.meta[,
                                                                                   x_y])
        }
        else {
            gmeans <- t(apply(categories, 1, function(row) {
                ...
                if (nrow(matches) == 1) {
                    # ADDED: Return a vector rather than a dataframe
                    return(c(matches$x, matches$y))
                }
                colnames(matches) <- c("x", "y")
                return(geosphere::geomean(matches))
            }))
            rownames(gmeans) <- categories[, 2] # original

            # ADDED: distm returns distance between samples in metres which we convert to kilometres.
            # Original function estimates a distance from the lat/long matrix
            m <- geosphere::distm(gmeans, fun=geosphere::distGeo)
            rownames(m) <- categories[, 2]
            colnames(m) <- categories[, 2]
            geo_mats[[categories[1, 1]]][[genetic_distance_method]] <- as.dist(m/1000)

            # This is the original code
            # geo_mats[[categories[1, 1]]][[genetic_distance_method]] <- stats::dist(gmeans) # original
        }
    }
        ...
        for (j in 1:length(gd[[i]])) {
            ...
            for (k in 1:length(gd[[i]][[j]])) {
                ...
                if (any(is.na(gd[[i]][[j]][[k]]))) {
                    ...
                    bad.entries <- which(rowSums(is.na(matgd)) >
                                             0)
                    gd[[i]][[j]][[k]] <- stats::as.dist(matgd[-bad.entries,
                                                              -bad.entries])
                    matgeod <- as.matrix(geo_mats[[tsampfacet]][[names(gd[[i]][[j]])[k]]])
                    tgeodist <- stats::as.dist(matgeod[-bad.entries,
                                                       -bad.entries])
                    warning("NAs detected in genetic distance data for facet: ",
                            paste(names(gd)[i], names(gd[[i]])[j], names(gd[[i]][[j]])[k]),
                            "\nlevels\t\n\t", paste0(row.names(matgeod)[bad.entries],
                                                     collapse = "\n\t"))
                }
                else {
                ...
                if (attr(gd[[i]][[j]][[k]], "Size") != 0) {
                    ibd[[i]][[j]][[k]] <- ade4::mantel.randtest(gd[[i]][[j]][[k]],
                                                                tgeodist, ...)
                }
                else {
                    warning("All genetic distance measures NA for facet",
                            paste(names(gd)[i], names(gd[[i]])[j], names(gd[[i]][[j]])[k]),
                            "\n\t")
                    ibd[[i]][[j]][[k]] <- NULL
                }
    ...
    x <- .update_citations(x, "Mantel209", "isolation_by_distance",
                           "Mantel test used to generate p-value for genetic vs geographic distance.")
    return(x)
}
...
#
.add.facets.snpR.data <- function(x, facets = NULL, b = FALSE){
    if(b) browser()
    ...
    for(k in 1:length(facet.list)){
        ...
        for(i in 1:nrow(sample.opts)){
            ...
            t.x <- snpR::genotypes(x)[,matches, drop = FALSE]
    ...
    x@stats <- data.table::as.data.table(dplyr::arrange(os, .snp.id, facet, subfacet))
        ...
        if(req == "genotypes"){
            ...
            loop.func <- function(x, opts, fname){
                ...
                for(i in 1:nrow(opts)){
                    ...
                    if(length(ident) > 0){
                        genotypes <- snpR::genotypes(x)[ident,, drop = FALSE]
...
.tabulate_genotypes <- function(x, mDat, verbose = F){
    ...
    .tab_func <- function(x, snp_form, mDat){
        ...
        for(i in 1:length(as)){
            ...
            if(length(hom) == 0){
                het <- b
                data.table::set(amat, j = i, value = rowSums(tmat[,het, with = FALSE]))
            }
            else{
                het <- b[b != hom]
                if(length(het) > 0){
                    if(data.table::is.data.table(tmat[,het, with = FALSE])){
                        data.table::set(amat, j = i, value = (tmat[,hom, with = FALSE] * 2) + rowSums(tmat[,het, with = FALSE]))
                    }
                    else{
                    ...
                    data.table::set(amat, j = i, value = (tmat[,hom, with = FALSE] * 2))
                }
            }
        }
...
.get.task.list <- function(x, facets, source = "stats"){
    ...
    for(i in 1:length(facets)){
        ...
        if(any(t.facet.type == "snp")){
            ...
            meta.to.use <- data.table::as.data.table(meta.to.use)
    ...
    for(i in 1:length(facets[[1]])){
        ...
        #=======================sample case========================
            ...
            if(facets[[2]][i] == "sample"){
                ...
                weights <- nrow(x) - matrixStats::colSums2(snpR::genotypes(x) == x@mDat)
            ...
            if(facets[[2]][i] == "snp"){
                ...
                for(j in 1:nrow(tl)){
                    snps_in_set <- which(unlist(do.call(paste, c(snp.meta(x)[,split.snp.part, drop = F], sep = "."))) ==
                                             tl[j,4])
                    tweights <- length(snps_in_set) - matrixStats::colSums2(snpR::genotypes(x)[snps_in_set,] == x@mDat)
            ...
            if(facets[[2]][i] == "complex"){
                ...
                for(j in 1:nrow(tl)){
                    ...
                    if(length(snps_in_set) > 0 & length(samps_in_set) > 0){
                        tweights <- length(snps_in_set) - matrixStats::colSums2(snpR::genotypes(x)[snps_in_set, samps_in_set, drop = F] == x@mDat)
...
.boot_ac <- function(x, n, facet){
    ...
    for(i in 1:n){
        ...
        shuff <- snpR::genotypes(x)[,sample(ncol(x), ncol(x), replace = TRUE)]
        shuff <- data.table::as.data.table(shuff)
        colnames(shuff) <- colnames(snpR::genotypes(x))
        tac <- vector("list", nrow(opts))
        glob_tab <- .tabulate_snpR::genotypes(shuff, x@mDat)
        ...
        # otherwise need to do for each facet level.
            ...
            for(j in 1:nrow(opts)){
                ...
                ttab <- .tabulate_snpR::genotypes(.fix..call(shuff[,..tm]), x@mDat)
...
.boot_as <- function(x, n, facet, ret_gs = FALSE, by = "sample"){
    ...
    for(i in 1:n){
        ...
        if(by == "sample"){
            ...
            shuff <- snpR::genotypes(x)[sample(nrow(x), nrow(x), replace = TRUE),]
        }
        shuff <- data.table::as.data.table(shuff)
        colnames(shuff) <- colnames(snpR::genotypes(x))
