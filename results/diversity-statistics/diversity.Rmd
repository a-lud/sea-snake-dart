...
object `meta`. This object is a list.
...
Next we load the VCF files into `R` and `snpRdata` objects. Similarly, we load
each species file separately and store it in a list. Some filtering is applied
which somewhat mirrors what we've already already applied. I'm including it here
as another approach to filtering a VCF file.
...
Below we estimate maf, pi, ho, he, pairwise Fst, Fis, HWE in addition to `Fst`.
We estimate all statistics by population.
