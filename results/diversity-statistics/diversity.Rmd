...
and between populations. The statistics are generated from the filtered VCF files
which have had the following filters applied:

- MAF $\leq$ 0.01
- LD $\leq$ 0.3
- For *H. stokesii* (WA), SNPs with PC1 loadings $\geq$ 0.04 (PC1 = batch effects)
...
Note that we've also removed the North and South QLD samples, as there aren't
...
samples_remove <- read_lines(here("data", "samples-remove.txt"))
...
```{r load-vcfs, results='hide', echo=TRUE}
...
Below we estimate π, H~o~, H~e~, F~IS~, HWE and pair wise F~ST~. We estimate
...
observed heterozygosity, expected heterozygosity and F~IS~.
...
within <- stats_within |>
    ...
    select(
        ...
        Population = subfacet,
        n_samples,
        `Nuc. diversity` = weighted_mean_pi,
...
We then manipulate the data a bit to get it to long-form so that we can plot
it easily.
